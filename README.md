...
Adds completed tasks with a timestamp to the "done" section of the daily note .txt file. It also queues a completed Todoist record with the `raycast-done` label in a detached worker, so the network request does not delay the Raycast confirmation. To find these records later, search Todoist for `%raycast-done` and select **Search completed tasks**. Saved filters cannot show completed tasks. A Todoist failure sends an immediate macOS notification and is written to `~/Library/Logs/raycast-note-scripts/todoist-worker.log`. If no task is provided, it moves the topmost task from the "now" section to the "done" section and updates the One Thing menubar app.
